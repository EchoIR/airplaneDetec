...
void validate_detector_recall(char *cfgfile, char *weightfile)
{
    ...
    list *plist = get_paths("/home/echo/EXERCISEs/airplaneDetec/yolov3/compar/infrared_test.txt");
    char **paths = (char **)list_to_array(plist);
...
void validate_detector_recall(char *cfgfile, char *weightfile)
{
    ...
    if(crop_flag){
        ...
        for(i = 0; i < m; ++i){
            ...
            if(im.w<=crop_size){
                wn=1;
            }else if((im.w-crop_size)%step==0){
                wn=(im.w-crop_size)/step+1;
            }else{
                wn = ceil(((float)im.w-crop_size)/step)+1;
     	        //hn = ceil(((float)im.h-crop_size)/step)+1;
            }
            if(im.h<=crop_size){
                hn=1;
            }else if((im.h-crop_size)%step==0){
                hn=(im.h-crop_size)/step+1;
            }else{
                hn = ceil(((float)im.h-crop_size)/step)+1;
            }
        
            //printf("wn:%d,hn:%d\n",wn,hn);
            //location; //记录的是每一个图像块的min_x,max_x.min_y,max_y

            int **location =(int **)calloc(wn*hn, sizeof(int *));//堆起来的二维数组
            for(j = 0;j<wn*hn;j++){
                location[j] = calloc(4, sizeof(int));
            }
            get_patchLocations(im, location, step, wn, hn,crop_size);
            ...
            for(j = 0;j<wn*hn;j++){
               dx = location[j][0];
               dy = location[j][2];
               im_patch = crop_image(im, dx, dy, crop_size, crop_size);
               image sized = letterbox_image(im_patch, net.w, net.h);
               network_predict(net, sized.data);
               get_region_boxes(l, im_patch.w, im_patch.h, net.w, net.h, thresh, probs, boxes, 1, 0, .5, 1);//就是将预测的结果填到boxes与probs中去
            ...
            int num_labels = 0;
            box_label *truth = read_boxes(labelpath, &num_labels);
            for(k = 0; k < l.w*l.h*l.n*wn*hn; ++k){
                if(probs_all[k][0] > thresh){
                    ++proposals;
                }
            }
            for (j = 0; j < num_labels; ++j) {
                ...
                float best_iou = 0;
                for(k = 0; k < l.w*l.h*l.n*wn*hn; ++k){
                    float iou = box_iou(boxes_all[k], t);
                    if(probs_all[k][0] > thresh && iou > best_iou){
                        best_iou = iou;
                    }
                }
                avg_iou += best_iou;
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        image sized = letterbox_image(im, net.w, net.h);//反正是先同比例的缩小放大，再就是生成与网络相同尺寸的图片（貌似用不够的话则用常数补充的来着）
