...
#这个脚本还会生成txt格式的yolo可识别的标签文件，转换自每一个用于训练或验证的图片对应的xml文件，txt格式的文件名称与对应的xml文件名相同，但是内容不同，扩展名不同
...
classes = ["airplane"]#因为我的数据集只有一个类别
...
def convert_annotation(image_id):
    ...
    in_file = open('/home/echo/EXERCISEs/airplaneDetec/Annotations/%s.xml'%(image_id))#与图片对应的xml文件所在的地址
    out_file = open('/home/echo/EXERCISEs/airplaneDetec/labels/%s.txt'%(image_id),'w') #与此xml对应的转换后的txt，这个txt的保存完整路径
...
image_ids = open('/home/echo/EXERCISEs/airplaneDetec/yolov3/train_final.txt').read().strip().split()  #如果是训练集数据打开这一行，注释下一行
#image_ids = open('/home/echo/EXERCISEs/airplaneDetec/yolov3/val.txt').read().strip().split()  #如果是验证数据集数据打开这一行，注释上一行
#list_file = open('%s_%s.txt'%(year, image_set), 'w')
list_file = open('/home/echo/EXERCISEs/airplaneDetec/yolov3/infrared_train1.txt', 'w')     #把结果写入到indrared_train.txt文件中，如果是训练集数据打开这一行，注释下一行
#list_file = open('/home/echo/EXERCISEs/airplaneDetec/yolov3/infrared_val.txt', 'w')     #把结果写入到indrared_train.txt文件中，如果是验证数据集数据打开这一行，注释上一行
for image_id in image_ids:
    #list_file.write('%s/VOCdevkit/VOC%s/JPEGImages/%s.jpg\n'%(wd, year, image_id))
    list_file.write('/home/echo/EXERCISEs/airplaneDetec/yolov3/JPEGImages/%s.jpg\n'%(image_id))  #把每一用于训练或验证的图片的完整的路径写入到infrared_train.txt中  这个文件会被voc.data yolo.c调用
    #convert_annotation(image_id)   #把图片的名称id传给函数，用于把此图片对应的xml中的数据转换成yolo要求的txt格式
list_file.close() #关闭文件

image_ids = open('/home/echo/EXERCISEs/airplaneDetec/yolov3/test_final.txt').read().strip().split()  #如果是训练集数据打开这一行，注释下一行
list_file = open('/home/echo/EXERCISEs/airplaneDetec/yolov3/infrared_test1.txt', 'w')     #把结果写入到indrared_train.txt文件中，如果是
for image_id in image_ids:
    #list_file.write('%s/VOCdevkit/VOC%s/JPEGImages/%s.jpg\n'%(wd, year, image_id))
    list_file.write('/home/echo/EXERCISEs/airplaneDetec/yolov3/JPEGImages/%s.jpg\n'%(image_id))  #把每一用于训练或验证的图片的完整的路径写入到infrared_train.txt中  这个文件会被voc.data yolo.c调用
    #convert_annotation(image_id)   #把图片的名称id传给函数，用于把此图片对应的xml中的数据转换成yolo要求的txt格式
